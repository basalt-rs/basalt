import { BasicSetupOptions, basicSetup } from '@uiw/react-codemirror';
...
import langs, { LanguageSyntax } from './langs';
...
export const getExtensions = (settings: EditorSettings, language?: LanguageSyntax) => {
    ...
    const lang = language ? langs[language] : undefined;
    if (lang) extensions.push(StreamLanguage.define(lang));
    if (settings.lineNumbers === 'relative') extensions.push(relativeLineNumbers());
    if (settings.indentGuides) extensions.push(indentationMarkers({ thickness: 3 }));
    if (settings.theme) extensions.push(themes[settings.theme]?.extension ?? themes.red);
    if (settings.minimap) {
        extensions.push(
